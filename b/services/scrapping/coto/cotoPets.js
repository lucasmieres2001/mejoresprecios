...
exports.cotoPets = async () => {
  const pageSize = 96; // Tamaño ideal según pruebas anteriores
  let offset = 0;
  let productos = [];

  while (true) {
    const url = `https://www.cotodigital.com.ar/sitios/cdigi/categoria/catalogo-hogar-mascotas/_/N-i0b09i?Nf=product.endDate%7CGTEQ%201.7545248E12%7C%7Cproduct.startDate%7CLTEQ%201.7545248E12&Nr=AND(product.sDisp_200:1004,product.language:espa%C3%B1ol,OR(product.siteId:CotoDigital))&showMoreIds=10011&No=${offset}&Nrpp=${pageSize}&format=json`;
    ...
    try {
      ...
      if (!foundProducts) {
        console.log('✅ Fin de MASCOTAS. No hay más productos.');
        break;
      }
      ...
      console.error('❌ Error MASCOTAS al obtener productos de Coto:', error.message);
      break;
    }
  }

  console.log(`🔍 Total MASCOTAS productos obtenidos de Coto: ${productos.length}`);
  return productos;
};
