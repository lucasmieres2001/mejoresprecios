...
exports.jumboTemplate = async (slug, tipo) => {
  try {
    const chunkSize = 50;
    let from = 0;
    ...
    while (from <= maxFrom && allProducts.length < maxProducts) {
      ...
      const url = `https://www.jumbo.com.ar/api/catalog_system/pub/products/search/${slug}?_from=${from}&_to=${to}`;
      try {
        ...
        console.log(`📥 Obtenidos ${allProducts.length} productos de jumbo "${tipo}" hasta ahora`);
        from += chunkSize;
    ...
    console.log(`✅ jumbo ${tipo}: ${allProducts.length} productos obtenidos`);
    return allProducts;

  } catch (err) {
    console.error(`Error en jumbo ${tipo}:`, err.message);
    return [];
  }
};
