  ...
  try {
    while (sessionCounter < MAX_SESSIONS && allProducts.length < REAL_TOTAL_PRODUCTS) {
      ...
      while (sessionActive && allProducts.length < REAL_TOTAL_PRODUCTS) {
        ...
        const url = `https://www.grupoelnene.com.ar/api/catalog_system/pub/products/search/almacen?_from=${from}&_to=${to}`;
    ...
    console.log(`✅ nene (almacén): ${allProducts.length} productos obtenidos en ${sessionCounter} sesiones`);
    return allProducts;

  } catch (err) {
    console.error('Error crítico en neneStore:', err.message);
    return allProducts;
  }
};
