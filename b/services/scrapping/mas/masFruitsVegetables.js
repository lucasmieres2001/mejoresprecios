...
const REAL_TOTAL_PRODUCTS = 21; // Valor real conocido
...
exports.masFruitsVegetables = async () => {
  const categoryId = 'verduras'; 
  let allProducts = [];
  let seenIds = new Set();
  let from = 0;
  ...
  try {
    while (sessionCounter < MAX_SESSIONS && allProducts.length < REAL_TOTAL_PRODUCTS) {
      ...
      while (sessionActive && allProducts.length < REAL_TOTAL_PRODUCTS) {
        ...
        const url = `https://www.masonline.com.ar/api/catalog_system/pub/products/search/?fq=${categoryId}/verduras-frescas&_from=${from}&_to=${to}`;
