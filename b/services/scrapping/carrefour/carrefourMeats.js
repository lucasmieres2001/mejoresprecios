  ...
  try {
    ...
    const initialBody = {
      ...
      variables: {
        ...
        map: "c",
        query: "Carnes-y-Pescados",
        orderBy: "OrderByScoreDESC",
        from: 0,
        to: pageSize - 1,
        selectedFacets: [
          { key: "c", value: "Carnes-y-Pescados" }
        ],
    ...
    const meatsList = allProducts.map(products => {
      ...
      const product = inferProductType(title, 'meats')
      return { title, price, img, distributor, product,url };
    }).filter(p => p.title && p.price && p.img);

    //console.log('Productos de Carrefour obtenidos:', meatsList);
    console.log(`Cantidad de productos de carne: ${meatsList.length} / ${total}`);

    return { carnes: meatsList };
